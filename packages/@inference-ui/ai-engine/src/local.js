...
class LocalAIEngine {
        ...
        try {
            ...
            switch (task.type) {
                ...
                case 'intent_detection':
                    output = await this.runTextClassification(task.input);
                    modelUsed = 'textClassification';
                    break;
                case 'form_validation':
                    output = await this.runFormValidation(task.input);
                    modelUsed = 'formValidation';
                    break;
                case 'autocomplete':
                    output = await this.runAutocomplete(task.input);
                    modelUsed = 'autocomplete';
                    break;
                case 'accessibility_check':
                    output = await this.runAccessibilityCheck(task.input);
     ...
     */
    async runTextClassification(text) {
     ...
     */
    async runAccessibilityCheck(_component) {
     ...
     */
    async loadModel(name, modelInfo) {
        try {
            if (!modelInfo) {
    ...
    async getCachedModels() {
        try {
            const cached = await AsyncStorage.getItem('@inference-ui/models');
     ...
     */
    postprocessClassification(_output) {
