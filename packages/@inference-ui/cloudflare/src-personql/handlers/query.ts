...
import type { PersonQLEnv, PersonQLQueryRequest, PersonQLQueryResponse } from '../types';
...
export async function handleQuery(
  ...
  try {
    ...
    if (cached) {
      const response: PersonQLQueryResponse = {
        data: cached,
        metadata: {
    ...
    if (result.data && !result.errors) {
      ctx.waitUntil(
        env.KV.put(cacheKey, JSON.stringify(result.data), {
...
async function executeGraphQLQuery(
  request: PersonQLQueryRequest,
  env: PersonQLEnv
): Promise<Pick<PersonQLQueryResponse, 'data' | 'errors'>> {
  ...
  if (query.includes('createPerson')) {
    ...
    await env.DB.prepare(
      `INSERT INTO persons (id, email, name, phone, metadata, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    )
      .bind(
        id,
