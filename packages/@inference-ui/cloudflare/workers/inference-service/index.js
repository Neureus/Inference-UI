...
class InferenceServiceWorker {
     ...
     */
    async processEvents(request) {
        let processed = 0;
        let failed = 0;
        const errors = [];
        try {
            // Process events in parallel
            const results = await Promise.allSettled(request.events.map(async (event) => {
              ...
              VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            `).bind(event.userId, event.sessionId || null, event.event, event.component || null, intent, sentiment, event.timestamp, JSON.stringify(event.properties || {})).run();
                }
                return true;
            }));
    ...
    async getAnalytics(request) {
        ...
        if (this.env.DB) {
            ...
            ORDER BY count DESC
            LIMIT 10
          `).bind(request.userId, request.startDate ? new Date(request.startDate).getTime() : Date.now() - 7 * 24 * 60 * 60 * 1000, request.endDate ? new Date(request.endDate).getTime() : Date.now()).all();
                    data = events.results;
                    break;
                case 'sessions':
    ...
    async getUsageMetrics(userId) {
        ...
        // Get user tier
        const user = await this.env.DB.prepare('SELECT tier FROM users WHERE id = ?').bind(userId).first();
    ...
    // Helper methods
    classifyIntent(event, _component) {
