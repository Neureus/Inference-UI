        ...
        try {
            ...
            switch (path) {
                ...
                case '/query':
                    ...
                    const queryReq = (await request.json());
                    return await handleQuery(queryReq, env, ctx);
                case '/direct':
                    if (request.method !== 'POST') {
                        return new Response('Method not allowed', { status: 405 });
                    }
                    const directReq = (await request.json());
                    return await handleDirect(directReq, env, ctx);
                case '/enrich':
                    if (request.method !== 'POST') {
                        return new Response('Method not allowed', { status: 405 });
                    }
                    const enrichReq = (await request.json());
                    return await handleEnrich(enrichReq, env, ctx);
                case '/cache':
                    if (request.method !== 'POST') {
                        return new Response('Method not allowed', { status: 405 });
                    }
                    const cacheReq = (await request.json());
